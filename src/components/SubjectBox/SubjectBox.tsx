...
export default function SubjectBox({ subject, href }: SubjectBoxProps) {
  const router = useRouter();
  const IconComponent = subjectIconMap[normalizeSubjectName(subject)];

  return (
    <Pressable
      className="items-center justify-center rounded-3xl bg-purpleCalm p-[10px]"
      style={{ width: "100%", aspectRatio: 1 }}
      onPress={() => router.navigate(href)}
    >
      <View className="items-center justify-center gap-[6px]">
        {IconComponent && <IconComponent width={48} height={48} />}
        <Text className="mt-[8px] text-center font-inter text-[14px] text-white" numberOfLines={2}>
          {subject}
        </Text>
      </View>
    </Pressable>
