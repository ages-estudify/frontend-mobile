...
export function useExam() {
  ...
  useEffect(() => {
    if (examId) {
      if (language) {
        // Errors are captured into `error` state inside createAttempt; swallow
        // the re-thrown rejection here so it does not become an unhandled rejection.
        createAttempt({ examId, language }).catch(() => {});
      } else {
        getLatestAttempt(examId).catch(() => {});
      }
    }
  }, [examId, day, language]);
