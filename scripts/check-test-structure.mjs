...
const DISALLOWED_PATTERNS = [/^app\//, /\/tests\//];
...
function main() {
  ...
  console.error("❌ Test files are in the wrong location:\n");
  for (const file of invalid) {
    console.error(`  - ${file}`);
  }

  console.error(
    "\nsrc/ tests must be co-located. app/ page tests must live in tests/. Use test/ only for shared setup and mocks."
  );
  process.exit(1);
}
